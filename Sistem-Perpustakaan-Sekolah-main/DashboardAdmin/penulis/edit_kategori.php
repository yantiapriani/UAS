...
if (isset($_POST['edit_kategori'])) {
    ...
    if (!$update_result) {
        die("Gagal mengupdate kategori: " . mysqli_error($conn));
    }
    // Redirect atau refresh halaman setelah mengupdate data
    header("Location: $_SERVER[PHP_SELF]");
    exit;
}
            ...
            <?php if (mysqli_num_rows($result) > 0): ?>
                <?php while ($row = mysqli_fetch_assoc($result)): ?>
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#editKategoriModal" data-kodekategori="<?php echo $row['kodekategori']; ?>" data-namakategori="<?php echo $row['nama_kategori']; ?>"><i class="fas fa-edit"></i> Edit</button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="edit_kodekategori" class="form-label">Kode Kategori</label>
                        <input type="text" class="form-control" id="edit_kodekategori" name="kode_kategori" readonly>
                    </div>
                    <div class="mb-3">
...
<script>
    ...
    editKategoriModal.addEventListener('show.bs.modal', function (event) {
        ...
        var kodekategori = button.getAttribute('data-kodekategori');
        var namakategori = button.getAttribute('data-namakategori');
        
        var modalKodekategori = editKategoriModal.querySelector('#edit_kodekategori');
        var modalNamakategori = editKategoriModal.querySelector('#edit_nama_kategori');
        
        modalKodekategori.value = kodekategori;
        modalNamakategori.value = namakategori;
    });
</script>
</body>
</html>
